...
import { buildQuantitiesProcurementReport } from '../../utils/quantitiesProcurementReport';
import { downloadSiaReport } from '../../utils/siaCaseReport';

export default function QuantitiesProcurementReport({ register, ewp, ewpId, registerId, busy, loadError, onReload }) {
  const [downloading, setDownloading] = useState(false);
  const [downloadError, setDownloadError] = useState('');
  const { report, error } = useMemo(() => {
    if (busy || loadError) return {};
    try { return { report: buildQuantitiesProcurementReport({ register, ewp, ewpId, registerId }) }; }
    catch (e) { return { error: e.message }; }
  }, [register, ewp, ewpId, registerId, busy, loadError]);
  async function download() {
    setDownloading(true); setDownloadError('');
    try {
      await downloadSiaReport(report, {
        title: 'Quantities & Procurement Report', header: 'GINFINA  /  ENGINEERING WORKBENCH',
        footer: 'Engineering quantities and procurement traceability', filename: 'Quantities-Procurement-Report', reference: 'EWP / quantity register reference',
      });
    } catch (e) { setDownloadError(e.message); }
    finally { setDownloading(false); }
  }
  return <Paper component="section" aria-label="Quantities & Procurement report" withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={{ minWidth: 0, overflowWrap: 'anywhere' }}>
    <Group justify="space-between" align="flex-start" mb="lg">
      <Box style={{ minWidth: 0 }}><Title order={3}>Quantities & Procurement Report</Title><Text size="sm" c="dimmed" mt={6}>Review the saved quantity and procurement details, then download the complete report as a PDF.</Text></Box>
    ...
    {busy && <Text role="status">Loading saved quantity and procurement records...</Text>}
