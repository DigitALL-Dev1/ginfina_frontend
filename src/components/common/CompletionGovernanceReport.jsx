...
import { buildCompletionGovernanceReport } from '../../utils/completionGovernanceReport';
import { downloadSiaReport } from '../../utils/siaCaseReport';

export default function CompletionGovernanceReport({ summary, ewpId, busy, loadError, onReload }) {
  const [downloading, setDownloading] = useState(false);
  const [downloadError, setDownloadError] = useState('');
  const { report, error } = useMemo(() => {
    if (busy || loadError) return {};
    try { return { report: buildCompletionGovernanceReport({ summary, ewpId }) }; }
    catch (e) { return { error: e.message }; }
  }, [summary, ewpId, busy, loadError]);
  async function download() {
    setDownloading(true); setDownloadError('');
    try {
      await downloadSiaReport(report, {
        title: 'Completion & Governance Report', header: 'GINFINA  /  ENGINEERING WORKBENCH',
        footer: 'Engineering completion checks and controlled EWP closure', filename: 'Completion-Governance-Report', reference: 'Engineering work package reference',
      });
    } catch (e) { setDownloadError(e.message); }
    finally { setDownloading(false); }
  }
  return <Paper component="section" aria-label="Completion & Governance report" withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={{ minWidth: 0, overflowWrap: 'anywhere' }}>
    <Group justify="space-between" align="flex-start" mb="lg">
      <Box style={{ minWidth: 0 }}><Title order={3}>Completion & Governance Report</Title><Text size="sm" c="dimmed" mt={6}>Review the saved completion and governance details, then download the complete report as a PDF.</Text></Box>
    ...
    {busy && <Text role="status">Loading saved completion and governance records...</Text>}
