...
import { loadEngineeringAssessmentReport } from '../../utils/siaEngineeringAssessmentReport';
import styles from '../../layouts/SIAEngineeringAssessmentLayout.module.css';

export default function SIAEngineeringAssessmentReport({ api, assessmentId, caseId }) {
  ...
  useEffect(() => {
    ...
    loadEngineeringAssessmentReport(api, assessmentId, caseId, controller.signal)
      .then(result => { if (!controller.signal.aborted) setReport(result); })
      .catch(e => { if (!controller.signal.aborted) setError(e.message); })
      .finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [api, assessmentId, caseId, refresh]);
  async function download() {
    setDownloading(true); setError('');
    try { await downloadSiaReport(report, { title: 'Engineering Assessment Report', footer: 'Saved engineering assessment records', filename: 'SIA-Engineering-Assessment-Report', reference: 'Assessment reference' }); }
    catch (e) { setError(e.message); } finally { setDownloading(false); }
  }
  return <Paper component="section" aria-label="Engineering Assessment report" withBorder radius="md" p={{ base: 'sm', sm: 'lg' }} style={{ overflowWrap: 'anywhere', minWidth: 0 }}>
    <Group justify="space-between" align="flex-start" mb="lg"><Box><Badge color="green" variant="light" mb="sm">Assessment report</Badge><Title order={3}>Engineering Assessment Report</Title>
      <Text size="sm" c="dimmed" mt={6}>Saved discipline assessments, findings, gaps and reviews. Statuses and review decisions are shown as recorded.</Text></Box>
    ...
    {loading && <Group py="lg"><Loader size="sm" color="green" /><Text size="sm">Loading engineering assessment records…</Text></Group>}
