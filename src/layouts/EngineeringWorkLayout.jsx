...
import {
  ...
  Progress, ScrollArea, Select, SimpleGrid, Slider, Stack, Table, Text,
  ...
  return (
    <Box p={{ base: 'md', md: 'xl' }} maw={1360} mx="auto">
      <Group justify="space-between" align="flex-start" mb="xl" wrap="wrap">
        <Box><Badge color="green" variant="light" mb="xs">Engineering Workbench</Badge><Title order={2}>Engineering Work</Title><Text c="dimmed" size="sm" mt={4}>Create an EWP from selected items in an immutable released SEB revision.</Text></Box>
      ...
      <Stack gap="lg">
        ...
        <Paper withBorder radius="lg" p={{ base: 'md', md: 'lg' }} style={surface}>
          ...
          <Grid>
            <Grid.Col span={{ base: 12, md: 4 }}><Select label="Project" placeholder={sourceLoading && !projects.length ? 'Loading released projects…' : 'Select project'} data={projects.map(item => ({ value: item.project_id, label: `${item.project_name}${item.project_code ? ` · ${item.project_code}` : ''}` }))} value={projectId || null} onChange={changeProject} searchable clearable disabled={sourceLoading && !projects.length} /></Grid.Col>
            <Grid.Col span={{ base: 12, md: 4 }}><Select label="Released SEB" placeholder={!projectId ? 'Select project first' : 'Select SEB'} data={sebOptions} value={sebId || null} onChange={changeSeb} searchable clearable disabled={!projectId || sourceLoading} /></Grid.Col>
            <Grid.Col span={{ base: 12, md: 4 }}><Select label="Released revision" placeholder={!sebId ? 'Select SEB first' : 'Select released revision'} data={revisionOptions} value={revisionId || null} onChange={changeRevision} searchable clearable disabled={!sebId || sourceLoading} /></Grid.Col>
          </Grid>
          ...
          {selectedReleasedSeb && <Paper withBorder p="sm" radius="md" mt="md" bg="#f3fbf6"><SimpleGrid cols={{ base: 1, sm: 4 }}><Info label="Project" value={selectedReleasedSeb.project_name} /><Info label="SEB" value={selectedReleasedSeb.seb_code} /><Info label="Released revision" value={selectedReleasedSeb.revision_no} /><Info label="Freeze snapshot" value={selectedReleasedSeb.freeze_snapshot_id} /></SimpleGrid></Paper>}
        </Paper>

        {selectedReleasedSeb && (
          <Paper withBorder radius="lg" style={surface}>
            <Group justify="space-between" align="flex-start" p="lg" wrap="wrap"><Box><Text fw={800}>6–7. Load and select frozen release items</Text><Text size="xs" c="dimmed">Select only the controlled SEB inputs required by this EWP.</Text></Box><Group><Badge color="gray" variant="light">{releaseItems.length} frozen</Badge><Badge color="green" variant="light">{selectedItemIds.length} selected</Badge></Group></Group>
            <Divider />
            <ScrollArea type="auto">
              <Table verticalSpacing="sm" miw={900}>
                <Table.Thead bg="#f7faf8"><Table.Tr><Table.Th w={48}><Checkbox aria-label="Select all frozen items" checked={releaseItems.length > 0 && selectedItemIds.length === releaseItems.length} indeterminate={selectedItemIds.length > 0 && selectedItemIds.length < releaseItems.length} onChange={event => toggleAllItems(event.currentTarget.checked)} /></Table.Th><Table.Th>Released item</Table.Th><Table.Th>Discipline</Table.Th><Table.Th>Review decision</Table.Th><Table.Th>Readiness</Table.Th><Table.Th>Source reference</Table.Th></Table.Tr></Table.Thead>
        ...
        {ewp && handoff && <>
          ...
          <Paper withBorder radius="lg" p={{ base: 'md', md: 'lg' }} style={surface}>
            <Group justify="space-between" align="flex-start" mb="lg" wrap="wrap"><Group align="flex-start"><ThemeIcon color="green" variant="light" radius="md" size={38}><IconShieldCheck size={20} /></ThemeIcon><Box><Text fw={800}>SEB Design Basis</Text><Text size="xs" c="dimmed">Exact frozen inputs linked to this EWP.</Text></Box></Group><Group><Button variant="default" leftSection={<IconEye size={16} />} onClick={() => setBasisModal(true)}>View selected inputs</Button><Checkbox color="green" checked={basisReviewed} onChange={event => setBasisReviewed(event.currentTarget.checked)} label="Design basis reviewed" /></Group></Group>
            <SimpleGrid cols={{ base: 2, sm: 4 }}><Info label="SEB" value={handoff.seb_code} /><Info label="Revision" value={handoff.revision} /><Info label="Readiness" value={<StatusBadge value={handoff.readiness} />} /><Info label="Linked inputs" value={inputLinks.length} /></SimpleGrid>
          ...
          <Paper withBorder radius="lg" p={{ base: 'md', md: 'lg' }} style={surface}>
            <Group justify="space-between" mb="lg"><Group align="flex-start"><ThemeIcon color="green" variant="light" radius="md" size={38}><IconFileCheck size={20} /></ThemeIcon><Box><Text fw={800}>EWP Definition</Text><Text size="xs" c="dimmed">Scope, ownership and planned dates.</Text></Box></Group><Button variant="subtle" color="green" leftSection={<IconEdit size={15} />} onClick={openEditEwp}>Edit EWP</Button></Group>
            <SimpleGrid cols={{ base: 1, sm: 2, lg: 4 }} spacing="lg"><Info label="EWP code" value={ewp.code} /><Info label="EWP name" value={ewp.name} /><Info label="Discipline" value={ewp.discipline} /><Info label="Lead engineer" value={ewp.lead} /><Box style={{ gridColumn: 'span 2' }}><Info label="Scope" value={ewp.scope} /></Box><Info label="Planned start" value={ewp.start} /><Info label="Planned finish" value={ewp.end} /></SimpleGrid>
          </Paper>
          <Paper withBorder radius="lg" style={surface}>
            <Group justify="space-between" p="lg" wrap="wrap"><Group align="flex-start"><ThemeIcon color="green" variant="light" radius="md" size={38}><IconProgressCheck size={20} /></ThemeIcon><Box><Text fw={800}>Engineering Work</Text><Text size="xs" c="dimmed">Create work activities, assign engineers and update progress.</Text></Box></Group><Group><Button variant="default" leftSection={<IconPlayerPlay size={16} />} loading={savingStatus} disabled={!basisReviewed || !work.length || ewp.status === 'IN_PROGRESS'} onClick={() => setEwpStatus('IN_PROGRESS')}>Start Engineering Work</Button><Button color="green" leftSection={<IconCheck size={16} />} loading={savingStatus} disabled={!allReadyForOutput || !basisReviewed || ewp.status === 'READY_FOR_OUTPUT'} onClick={() => setEwpStatus('READY_FOR_OUTPUT')}>Mark READY FOR OUTPUT</Button><Button color="green" variant="light" leftSection={<IconPlus size={16} />} onClick={openNewActivity}>Add Engineering Work</Button></Group></Group>
            <Divider />
            <ScrollArea type="auto"><Table verticalSpacing="sm" miw={900}><Table.Thead bg="#f7faf8"><Table.Tr><Table.Th>Work order</Table.Th><Table.Th>Activity</Table.Th><Table.Th>Engineer(s)</Table.Th><Table.Th>Planned dates</Table.Th><Table.Th>Status</Table.Th><Table.Th>Progress</Table.Th><Table.Th></Table.Th></Table.Tr></Table.Thead><Table.Tbody>{work.map(item => <Table.Tr key={item.id}><Table.Td><Text fw={750} size="sm">{item.code}</Text></Table.Td><Table.Td><Text size="sm">{item.title}</Text></Table.Td><Table.Td>{item.engineer ? <Group gap="xs"><IconUsers size={15} color="#4b6b59" /><Text size="sm">{item.engineer}</Text></Group> : <Badge color="orange" variant="light">Unassigned</Badge>}</Table.Td><Table.Td><Text size="xs">{item.start || '—'} → {item.end || '—'}</Text></Table.Td><Table.Td><StatusBadge value={item.status} /></Table.Td><Table.Td w={150}><Group gap="xs" wrap="nowrap"><Progress value={item.progress} color={item.progress === 100 ? 'green' : 'blue'} w={90} /><Text size="xs" fw={700}>{item.progress}%</Text></Group></Table.Td><Table.Td><Button size="xs" variant="subtle" color="green" onClick={() => openActivity(item)}>Manage</Button></Table.Td></Table.Tr>)}</Table.Tbody></Table></ScrollArea>
      ...
      <Modal opened={basisModal} onClose={() => setBasisModal(false)} title={<Text fw={800}>Selected SEB Design Basis Inputs</Text>} size="xl" centered radius="lg">
        {handoff && <Stack gap="lg"><Group><StatusBadge value={handoff.status} /><StatusBadge value={handoff.readiness} /><Badge variant="outline">{handoff.seb_code} / {handoff.revision}</Badge></Group><ScrollArea type="auto"><Table withTableBorder verticalSpacing="sm"><Table.Thead><Table.Tr><Table.Th>SEB item</Table.Th><Table.Th>Discipline</Table.Th><Table.Th>Decision</Table.Th><Table.Th>Readiness</Table.Th><Table.Th>Evidence</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{handoff.items.map(item => <Table.Tr key={item.id}><Table.Td><Text fw={700} size="sm">{item.fact}</Text><Text size="xs" c="dimmed">{item.id}</Text></Table.Td><Table.Td>{item.discipline}</Table.Td><Table.Td><StatusBadge value={item.decision} /></Table.Td><Table.Td><StatusBadge value={item.readiness} /></Table.Td><Table.Td><Group gap="xs"><IconLink size={14} /><Text size="sm">{item.evidence}</Text></Group></Table.Td></Table.Tr>)}</Table.Tbody></Table></ScrollArea><SimpleGrid cols={{ base: 1, sm: 2 }}><Paper withBorder p="md" radius="md"><Text fw={750} mb="xs">Conditions</Text>{handoff.conditions.length ? handoff.conditions.map(item => <Text key={item} size="sm" mb={6}>• {item}</Text>) : <Text size="sm" c="dimmed">None</Text>}</Paper><Paper withBorder p="md" radius="md"><Text fw={750} mb="xs">Constraints</Text>{handoff.constraints.length ? handoff.constraints.map(item => <Text key={item} size="sm" mb={6}>• {item}</Text>) : <Text size="sm" c="dimmed">None</Text>}</Paper></SimpleGrid></Stack>}
      </Modal>

      <Modal opened={ewpModal} onClose={() => setEwpModal(false)} title={<Text fw={800}>{editingEwpId ? 'Edit EWP' : 'Create EWP from selected release items'}</Text>} size="lg" centered radius="lg">
        <Stack>
          ...
          <Grid><Grid.Col span={6}><DatePickerInput label="Planned start" value={ewpForm.start} onChange={value => { setEwpForm(form => ({ ...form, start: value })); }} /></Grid.Col><Grid.Col span={6}><DatePickerInput label="Planned finish" value={ewpForm.end} onChange={value => { setEwpForm(form => ({ ...form, end: value })); }} /></Grid.Col></Grid>
          <Group justify="flex-end" mt="md"><Button variant="default" onClick={() => setEwpModal(false)} disabled={savingEwp}>Cancel</Button><Button color="green" loading={savingEwp} disabled={!ewpForm.code.trim() || !ewpForm.name.trim()} onClick={saveEwp}>{editingEwpId ? 'Save changes' : 'Create EWP'}</Button></Group>
        </Stack>
      </Modal>

      <Modal opened={activityModal} onClose={() => setActivityModal(false)} title={<Text fw={800}>{editingActivity ? 'Update Engineering Work' : 'Add Engineering Work'}</Text>} size="lg" centered radius="lg">
        <Stack><Grid><Grid.Col span={{ base: 12, sm: 4 }}><TextInput label="Work order" required value={activityForm.code} onChange={event => { const value = event.currentTarget.value; setActivityForm(form => ({ ...form, code: value })); }} /></Grid.Col><Grid.Col span={{ base: 12, sm: 8 }}><TextInput label="Activity" required value={activityForm.title} onChange={event => { const value = event.currentTarget.value; setActivityForm(form => ({ ...form, title: value })); }} /></Grid.Col></Grid><TextInput label="Assigned engineer(s)" description="Separate multiple engineers with commas" value={activityForm.engineer} onChange={event => { const value = event.currentTarget.value; setActivityForm(form => ({ ...form, engineer: value })); }} /><Grid><Grid.Col span={6}><DatePickerInput label="Planned start" value={activityForm.start} onChange={value => { setActivityForm(form => ({ ...form, start: value })); }} /></Grid.Col><Grid.Col span={6}><DatePickerInput label="Planned finish" value={activityForm.end} onChange={value => { setActivityForm(form => ({ ...form, end: value })); }} /></Grid.Col></Grid><Select label="Work status" data={STATUSES.map(value => ({ value, label: value.replaceAll('_', ' ') }))} value={activityForm.status} onChange={value => setActivityForm(form => ({ ...form, status: value || 'NOT_STARTED', progress: value === 'COMPLETED' ? 100 : form.progress }))} /><Box><Group justify="space-between" mb="xs"><Text size="sm" fw={600}>Progress</Text><Text size="sm" fw={750}>{activityForm.progress}%</Text></Group><Slider color="green" value={Number(activityForm.progress)} onChange={value => setActivityForm(form => ({ ...form, progress: value, status: value === 100 ? 'COMPLETED' : value > 0 && form.status === 'NOT_STARTED' ? 'IN_PROGRESS' : form.status }))} marks={[{ value: 0, label: '0%' }, { value: 50, label: '50%' }, { value: 100, label: '100%' }]} mb="xl" /></Box><Group justify="flex-end" mt="md"><Button variant="default" onClick={() => setActivityModal(false)} disabled={savingActivity}>Cancel</Button><Button color="green" loading={savingActivity} onClick={saveActivity}>{editingActivity ? 'Update work' : 'Add work'}</Button></Group></Stack>
      </Modal>
    </Box>
  );
}
