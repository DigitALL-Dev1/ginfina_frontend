...
import {
  ...
  Paper, Progress, ScrollArea, Select, SimpleGrid, Stack, Table, Text,
      ...
      <SimpleGrid cols={{ base: 2, sm: 3, lg: 6 }} spacing="xs">
        {FLOW.map(([label, detail], index) => {
          ...
          return <Group key={label} gap="xs" wrap="nowrap" p="xs" style={{ borderRadius: 9, background: current ? '#edf8f1' : 'transparent' }}>
            ...
            <Box style={{ minWidth: 0 }}><Text size="xs" fw={current ? 800 : 650} truncate>{label}</Text><Text size="10px" c="dimmed" truncate>{detail}</Text></Box>
          </Group>;
        })}
      </SimpleGrid>
    </Paper>
...
export default function SEBEWBHandoffLayout() {
  ...
  return <Box p={{ base: 'md', md: 'xl' }} maw={1280} mx="auto">
    <Group justify="space-between" align="flex-start" mb="xl" wrap="wrap"><Box><Badge color="green" variant="light" mb="xs">SEB → EWP</Badge><Title order={2} c="#16261e">EWP Handoff</Title><Text c="dimmed" size="sm" mt={4}>Select released engineering facts and pass their frozen context into an Engineering Work Package.</Text></Box><ThemeIcon size={48} radius="lg" color="green" variant="light"><IconPackageExport size={25} /></ThemeIcon></Group>
    <Stack gap="lg">
      ...
      <Paper withBorder p={{ base: 'md', md: 'xl' }} radius="lg" style={surface} mih={390}>
        ...
        {step === 5 && <Stack gap="lg">
          ...
          <ScrollArea type="auto"><Table withTableBorder verticalSpacing="sm" miw={900}><Table.Thead bg="#f7faf8"><Table.Tr><Table.Th>Fact</Table.Th><Table.Th>Discipline</Table.Th><Table.Th>Readiness</Table.Th><Table.Th>Conditions</Table.Th><Table.Th>Blockers / gaps</Table.Th><Table.Th>Evidence</Table.Th><Table.Th></Table.Th></Table.Tr></Table.Thead><Table.Tbody>{selectedItems.map(item => <Table.Tr key={item.id}><Table.Td><Text size="sm" fw={700}>{item.fact_name}</Text><Text size="xs" c="dimmed">{item.fact_id}</Text></Table.Td><Table.Td>{item.discipline}</Table.Td><Table.Td><StatusBadge value={item.readiness} /></Table.Td><Table.Td>{conditionFor(item.id).length}</Table.Td><Table.Td>{blockerFor(item.id).length}</Table.Td><Table.Td>{evidenceFor(item.id).length}</Table.Td><Table.Td><Button size="xs" variant="subtle" color="green" onClick={() => setDetailItem(item)}>View details</Button></Table.Td></Table.Tr>)}</Table.Tbody></Table></ScrollArea>
