...
import { createIfcRelease } from '../services/ifcReleaseService';
...
export default function EngineeringReleaseLayout() {
  const [selectedDoc, setSelectedDoc] = useState('GL-UNICEF-BOR-SLD-001');
  const [releaseType, setReleaseType] = useState('IFC');
  const [releaseNote, setReleaseNote] = useState('');
  const [approvedBaseline, setApprovedBaseline] = useState('BASELINE-UNI-BOR-ELE-002');
  const [includedRevisions, setIncludedRevisions] = useState('UNI-BOR-ELE-SLD-001 (R2.0), UNI-BOR-ELE-CALC-001 (R1.1)');
  ...
  const handleRelease = async () => {
    ...
    if (!approvedBaseline.trim()) {
      notifications.show({
        title: 'Validation Error',
        message: 'Approved baseline is required',
        color: 'red',
      });
      return;
    }
    if (!includedRevisions.trim()) {
      notifications.show({
        title: 'Validation Error',
        message: 'Included revisions are required',
        color: 'red',
      });
      return;
    }
    ...
    try {
      const releaseData = {
        release_type: `Full Package ${releaseType}`,
        approved_baseline: approvedBaseline.trim(),
        included_revisions: includedRevisions.trim(),
      ...
      await createIfcRelease(releaseData);

      notifications.show({
        title: 'Baseline Released',
        message: `${releaseType} baseline released and immutable hash published.`,
        color: 'green',
      });

      // Clear the release note field after successful submission
      setReleaseNote('');
    } catch (error) {
      notifications.show({
        title: 'Error',
        message: 'Failed to create IFC release: ' + error.message,
        color: 'red',
      });
    } finally {
  ...
  const handleReturn = () => {
    notifications.show({
      ...
      message: 'Package returned for revision before release.',
      color: 'orange',
    });
  };
        ...
        <Group justify="space-between" align="flex-start" wrap="wrap" gap="md">
          <Box style={{ flex: 1 }}>
            <Title order={1} fz={{ base: 20, sm: 24 }} fw={700} c="#111827">
              Controlled Engineering Release - IFR / IFA / IFC
            </Title>
            <Text size="sm" c="#6b7280" mt={4} maw={680}>
              Create and issue an immutable IFR, IFA or IFC engineering release containing exact approved revisions, baseline hash, approval evidence and distribution scope.
            </Text>
          </Box>
          <Button
            color="green"
          ...
          <Paper p="md" style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <Box>
              ...
              <Grid gutter="md">
                ...
                <Grid.Col span={6}>
                  <Box>
                    ...
                    <Select
                      ...
                      styles={inputSt}
                    />
                    <Text size="11px" c="#9ca3af" mt={4}>Select · IFR / IFA / IFC</Text>
                  </Box>
                </Grid.Col>
                ...
                <Grid.Col span={6}>
                  <Box>
                    <Text size="xs" fw={600} c="#374151" mb={4}>Approved baseline <Text span c="red">*</Text></Text>
                    <TextInput value={approvedBaseline} readOnly styles={roSt} />
                    <Text size="11px" c="#9ca3af" mt={4}>Read-only · Must be valid and current</Text>
                  </Box>
                </Grid.Col>
                ...
                <Grid.Col span={6}>
                  <Box>
                    <Text size="xs" fw={600} c="#374151" mb={4}>Included revisions <Text span c="red">*</Text></Text>
                    <TextInput value={includedRevisions} readOnly styles={roSt} />
                    <Text size="11px" c="#9ca3af" mt={4}>Read-only manifest · Approval-bounded</Text>
                  </Box>
                </Grid.Col>
                ...
                <Grid.Col span={6}>
                  <Box>
                    <Text size="xs" fw={600} c="#374151" mb={4}>Release note <Text span c="red">*</Text></Text>
                    <Textarea
                      value={releaseNote}
