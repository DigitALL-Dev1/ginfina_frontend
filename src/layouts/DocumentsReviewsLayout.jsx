...
import DocumentsReviewsReport from '../components/common/DocumentsReviewsReport';
...
export default function DocumentsReviewsLayout() {
  ...
  return (
    <Box className={styles.page} p={{ base: 'sm', sm: 'md', md: 'xl' }} maw={1480} mx="auto">
      <Group justify="space-between" align="flex-start" mb="xl">
        <Box className={styles.headingText}><Text className={styles.eyebrow} mb={8}>Engineering workbench / Technical review</Text><Title order={2}>Documents &amp; Reviews</Title><Text size="sm" c="dimmed" mt={6}>Create controlled document revisions and complete traceable technical review.</Text></Box>
      ...
      <Stack gap="lg">
        ...
        <Box className={styles.context}>
          ...
          <Grid align="flex-end">
            <Grid.Col span={{ base: 12, sm: 6, lg: 5 }}><Select classNames={{ dropdown: styles.dropdown }} label="EWP" placeholder="Select an engineering work package" data={ewps.map(item => ({ value: item.id, label: `${item.code} / ${item.name}` }))} value={ewpId || null} onChange={selectEwp} searchable clearable disabled={loading || saving} /></Grid.Col>
            <Grid.Col span={{ base: 12, sm: 6, lg: 5 }}><Select classNames={{ dropdown: styles.dropdown }} label="Deliverable" placeholder={!ewpId ? 'Select EWP first' : 'Select a review-ready deliverable'} data={ewpDeliverables.map(item => ({ value: item.id, label: `${item.code} / ${item.name}` }))} value={deliverableId || null} onChange={selectDeliverable} searchable clearable disabled={!ewpId || loading || saving} /></Grid.Col>
            <Grid.Col span={{ base: 12, lg: 2 }}><Button fullWidth color="green" leftSection={<IconPlus size={16} />} disabled={!deliverableId || loading || saving} onClick={prepareDocument}>Document</Button></Grid.Col>
          </Grid>
          ...
          <Stack gap="lg" className={styles.main}>
            ...
            {document && !loading && <>
              <Box className={styles.hero}>
                ...
                <Divider my="lg" />
                <Group justify="space-between" align="center"><Box style={{ minWidth: 0 }}><Text size="xs" c="dimmed">Current file</Text><Text size="sm" fw={600} className={styles.title}>{currentRevision?.file_name || 'No file uploaded'}</Text></Box><Group className={styles.actions} gap="xs">
              ...
              <Tabs value={tab} onChange={setTab} color="green" keepMounted={false}>
                ...
                <Tabs.Panel value="review"><Box className={styles.panel}>
                  ...
                  <Stack gap="md">{reviewers.map(item => <Box key={item.id} className={styles.reviewer}><Group justify="space-between" align="flex-start"><Group align="flex-start" gap="sm"><ThemeIcon size={36} radius="xl" color={item.status === 'COMPLETED' ? 'green' : 'gray'} variant="light"><Text size="sm" fw={750}>{item.reviewer_name?.charAt(0)?.toUpperCase() || 'R'}</Text></ThemeIcon><Box><Text size="sm" fw={700}>{item.reviewer_name}</Text><Text size="xs" c="dimmed">{item.reviewer_role}</Text></Box></Group><StatusBadge value={item.decision || item.status} /></Group>
                    ...
                    {!reviewLocked && <Group className={styles.actions} justify="flex-end" mt="sm"><Button size="xs" variant="subtle" color="green" disabled={saving} onClick={() => openDecision(item)}>{item.status === 'COMPLETED' ? 'Update decision' : 'Record decision'}</Button></Group>}
                ...
                <Tabs.Panel value="comments"><Box className={styles.panel}>
                  ...
                  <Stack>{visibleComments.map(item => <Box key={item.id} className={styles.comment} data-closed={item.status === 'CLOSED'}>
                    ...
                    <Group className={styles.actions} justify="flex-end" mt="sm">{commentLocked(item) ? <Group gap={4}><IconLock size={12} /><Text size="xs" c="dimmed">Locked revision</Text></Group> : <Button size="xs" variant="light" color="green" disabled={saving} onClick={() => openResponse(item)}>{item.response ? 'Update response' : 'Respond'}</Button>}</Group>
              ...
              <Box className={styles.completion}>
                <Group justify="space-between" align="flex-start" gap="lg"><Box className={styles.completionText}><Text fw={750}>{reviewLocked ? 'Technical review recorded' : 'Ready to complete review?'}</Text><Text size="sm" c="dimmed" mt={4}>{reviewLocked ? 'The review decisions and responses are retained with this revision.' : 'Resolve these requirements before sending this revision for approval.'}</Text>
                  {!reviewLocked && <Stack gap={7} mt="md">{[[allReviewersComplete, 'All assigned reviewers have finished'], [!blockingDecision, 'No change-required or rejected decisions'], [allCommentsClosed, 'All review comments are closed']].map(([done, label]) => <Group key={label} gap={7} wrap="nowrap"><ThemeIcon size={18} radius="xl" variant="light" color={done ? 'green' : 'orange'}>{done ? <IconCheck size={12} /> : <IconAlertTriangle size={12} />}</ThemeIcon><Text size="xs">{label}</Text></Group>)}</Stack>}
