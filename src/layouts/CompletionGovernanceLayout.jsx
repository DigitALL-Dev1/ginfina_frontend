...
import styles from './CompletionGovernanceLayout.module.css';
import { notifications } from '@mantine/notifications';
import { IconAlertTriangle, IconCheck, IconLock, IconRefresh, IconShieldCheck } from '@tabler/icons-react';

import CompletionGovernanceReport from '../components/common/CompletionGovernanceReport';
...
export default function CompletionGovernanceLayout() {
  ...
  useEffect(() => {
    ...
    setListLoading(true);
    request('/ewps', { signal: controller.signal }).then(rows => { if (!controller.signal.aborted) setEwps(rows); }).catch(e => { if (e.name !== 'AbortError') setError(e.message); })
  ...
  useEffect(() => {
    ...
    request(`/ewps/${encodeURIComponent(ewpId)}/summary`, { signal: controller.signal }).then(data => { if (!controller.signal.aborted) setSummary(data); })
  ...
  return <Box className={styles.page} p={{ base: 'sm', sm: 'md', md: 'xl' }} maw={1380} mx="auto">
    <Group justify="space-between" align="flex-start" mb="xl"><Box className={styles.headingText}><Badge color="green" variant="light" mb="xs">Engineering Workbench</Badge>
    ...
    <Stack gap="lg">
      <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Group className={styles.actions} align="flex-end">
        <Select classNames={{ dropdown: styles.dropdown }} style={{ flex: 1 }} label="Engineering Work Package" placeholder={listLoading ? 'Loading EWPs...' : 'Select EWP'} searchable clearable
      ...
      {summary && <>
        <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Group justify="space-between"><Box><Title order={3}>{summary.ewp.ewp_code}</Title><Text c="dimmed" size="sm">{summary.ewp.ewp_name}</Text></Box><Status value={summary.completion_status} /></Group>
        ...
        <Paper withBorder radius="lg" style={surface}><Box p={{ base: 'sm', sm: 'lg' }}><Title order={4}>Completion checklist</Title><Text size="sm" c="dimmed">Calculated from saved records. Resolve failed checks in their originating module.</Text></Box><Divider />
          <Box className={styles.tableWrap}><Table className={styles.checklist} verticalSpacing="sm"><Table.Thead><Table.Tr><Table.Th>Requirement</Table.Th><Table.Th>Result</Table.Th><Table.Th>Status</Table.Th><Table.Th>Source</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>{checks.map(c => <Table.Tr key={c.id}><Table.Td data-label="Requirement"><Box><Text size="sm" fw={650}>{c.check_name}</Text><Text size="xs" c="dimmed">{c.mandatory ? 'Mandatory' : 'Optional'} &middot; {c.total} records</Text></Box></Table.Td>
              <Table.Td data-label="Result"><Text size="sm">{c.remarks}</Text></Table.Td><Table.Td data-label="Status"><Status value={c.status} /></Table.Td><Table.Td data-label="Source">{modules[c.check_type] && <Button component="a" href={`/ginfina/ewp/${modules[c.check_type]}`} size="compact-xs" variant="subtle">Open module</Button>}</Table.Td></Table.Tr>)}</Table.Tbody>
          </Table></Box></Paper>
        <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Group justify="space-between" mb="md"><Title order={4}>Conditions, blockers & actions</Title><Button color="green" variant="light" disabled={closed || saving} onClick={() => openAction('actions')}>Add action</Button></Group>
          {!summary.obligations.length ? <Text size="sm" c="dimmed">No conditions, blockers or recorded actions for this EWP.</Text> : <Stack>{summary.obligations.map(o => <Paper key={o.id} withBorder p="md" radius="md"><Group justify="space-between" align="flex-start"><Box className={styles.obligationText}><Text size="xs" c="dimmed">{o.kind}</Text><Text fw={600} size="sm">{o.title}</Text>{o.resolution && <Text size="xs" mt={4}>Recorded by {o.resolution.actor}: {o.resolution.comment}</Text>}</Box><Status value={o.status} />{o.status === 'OPEN' && !closed && (o.managed_in === 'MANAGEMENT' ? <Button component="a" href="/ginfina/ewp/management-administration" size="xs" variant="light">Open Management</Button> : <Button size="xs" variant="light" onClick={() => openAction('resolutions', o)}>Resolve</Button>)}</Group></Paper>)}</Stack>}
        </Paper>
        <SimpleGrid cols={{ base: 1, lg: 2 }}>
          <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Title order={4} mb="md">Governance verification</Title><Stack>
    ...
    <Modal fullScreen={isMobile} classNames={{ content: styles.modal }} closeButtonProps={{ "aria-label": "Close dialog" }} opened={Boolean(modal)} onClose={() => { if (!saving) { setModal(''); setError(''); } }} title={actionLabels[modal]} centered closeOnClickOutside={!saving} closeOnEscape={!saving} withCloseButton={!saving}>
      <Stack>{error && <Alert color="red">{error}</Alert>}
        ...
        {modal === 'resolutions' && <><Text size="sm" fw={600}>{selected?.title}</Text><Select classNames={{ dropdown: styles.dropdown }} label="Resolution" data={selected?.kind === 'CONDITION' ? [{ value: 'CLOSED', label: 'Closed' }, { value: 'ACCEPTED', label: 'Formally accepted' }] : [{ value: 'CLOSED', label: 'Closed' }]} value={form.status} onChange={value => setForm(f => ({ ...f, status: value || 'CLOSED' }))} /></>}
        ...
        <Group className={styles.actions} justify="flex-end"><Button variant="default" disabled={saving} onClick={() => { setModal(''); setError(''); }}>Cancel</Button><Button color="green" loading={saving} disabled={!form.actor.trim() || !form.comment.trim() || (modal === 'actions' && !form.title.trim())} onClick={save}>Save decision</Button></Group>
      </Stack>
    </Modal>
  </Box>;
}
