...
export default function AuthorityMatrixLayout() {
  ...
  /* Left filters */
  const [userRole, setUserRole]       = useState('Electrical Lead');
  const [discipline, setDiscipline]   = useState('Electrical');
  const [authorityLevel, setAuthorityLevel] = useState('IFC Approver');

  const filtered = useMemo(() => {
    const q = topSearch.trim().toLowerCase();
    return PILOT_AUTHORITIES.filter((r) => {
  ...
  const handleExport = () =>
    notifications.show({ title: 'Export initiated', message: `Exporting authority matrix records…`, color: 'green' });

  const handlePublish = () =>
    notifications.show({ title: 'Authority Matrix Published', message: 'Authority matrix changes saved and published.', color: 'green' });
        ...
        <Group justify="space-between" align="flex-start" wrap="wrap" gap="md">
          <Box style={{ flex: 1 }}>
            <Title order={1} fz={{ base: 20, sm: 24 }} fw={700} c="#111827">
              Engineering Authority and Reviewer Matrix
            </Title>
            <Text size="sm" c="#6b7280" mt={4} maw={680}>
              Define who may review, verify and approve specific disciplines, stages and package types, including competency, expiry and conflict controls.
            </Text>
          </Box>
          <Button
            color="green"
        ...
        <Group justify="space-between" align="flex-end" wrap="wrap" gap="md">
          <Group align="flex-end" gap="md" style={{ flex: 1, minWidth: 300 }}>
            ...
            <Box style={{ width: 160 }}>
              ...
              <Select
                data={['Active / Open', 'All', 'Active', 'Scoped', 'Expired']}
                value={topStatus}
            ...
            <Box style={{ width: 190 }}>
              ...
              <Select
                data={['My Priority Work', 'All Roles', 'Electrical Reviewers', 'Managers']}
                value={savedView}
          ...
          <Paper p="md" style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <Box>
              ...
              <Stack gap="md">
                {/* User / role */}
                <Box>
                  <Text size="xs" fw={600} c="#374151" mb={4}>User / role <Text span c="red">*</Text></Text>
                  <Select
                    data={['Electrical Lead', 'Engineering Manager', 'Electrical Reviewer', 'Consultant - ABC', 'Procurement Lead']}
                    value={userRole}
                    onChange={(v) => setUserRole(v || 'Electrical Lead')}
                    styles={inputSt}
                  />
                  <Text size="11px" c="#9ca3af" mt={4}>People Select · Authorised identity</Text>
                </Box>
                ...
                <Box>
                  ...
                  <Select
                    data={['Electrical', 'Civil', 'Structural', 'Mechanical', 'All']}
                    value={discipline}
                ...
                {/* Authority level */}
                <Box>
                  <Text size="xs" fw={600} c="#374151" mb={4}>Authority level <Text span c="red">*</Text></Text>
                  <Select
                    data={['IFC Approver', 'Reviewer', 'Author', 'Gate Authority']}
                    value={authorityLevel}
                    onChange={(v) => setAuthorityLevel(v || 'IFC Approver')}
                    styles={inputSt}
                  />
                  <Text size="11px" c="#9ca3af" mt={4}>Select · Controlled authority</Text>
                </Box>
              </Stack>
            </Box>
        ...
        <Grid.Col span={{ base: 12, md: 8.2, lg: 8.5 }}>
          <Paper p="md" style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, minHeight: 380, display: 'flex', flexDirection: 'column' }}>
            <Group justify="space-between" align="center" mb="sm">
              <Title order={4} fw={700} fz={15} c="#111827">Engineering Authority and Reviewer Matrix</Title>
              <Badge
                size="sm"
                radius="xl"
