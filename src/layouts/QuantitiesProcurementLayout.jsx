...
import {
  ...
  Paper, Select, SimpleGrid, Stack, Table, Text, Textarea, TextInput,
  ThemeIcon, Title,
} from '@mantine/core';
import { useMediaQuery } from '@mantine/hooks';
import styles from './QuantitiesProcurementLayout.module.css';
...
import QuantitiesProcurementReport from '../components/common/QuantitiesProcurementReport';
...
function QuantityTable({ rows, editable = false, onEdit, onRemove, onSource, saving }) {
  return <Box className={styles.tableWrap}><Table className={styles.recordTable} verticalSpacing="sm" highlightOnHover><Table.Thead><Table.Tr>
    ...
    <Table.Td data-label="Item / specification"><Box><Text size="sm" fw={650}>{row.item}</Text><Text size="xs" c="dimmed">{row.specification}</Text>{row.description && <Text size="xs">{row.description}</Text>}</Box></Table.Td>
    <Table.Td data-label="Quantity">{row.quantity.toLocaleString()}</Table.Td><Table.Td data-label="Unit">{row.unit}</Table.Td>
    <Table.Td data-label="Released source"><Button variant="subtle" size="compact-xs" onClick={() => onSource(row.source)}>{row.source.document_code} / {row.source.revision_no}</Button></Table.Td>
    <Table.Td data-label="Drawing reference"><Box><Text size="sm">{row.source_reference}</Text><Text size="xs" c="dimmed">{row.derivation}</Text></Box></Table.Td>
    {editable && <Table.Td data-label="Actions"><Group className={styles.rowActions} gap="xs"><Button size="compact-xs" variant="light" disabled={saving} onClick={() => onEdit(row)}>Edit</Button><Button size="compact-xs" color="red" variant="subtle" disabled={saving} onClick={() => onRemove(row)}>Remove</Button></Group></Table.Td>}
  </Table.Tr>)}</Table.Tbody></Table></Box>;
}
...
export default function QuantitiesProcurementLayout() {
  ...
  return <Box className={styles.page} p={{ base: 'sm', sm: 'md', md: 'xl' }} maw={1380} mx="auto"><Stack gap="lg">
    ...
    <Paper withBorder p={{ base: 'sm', sm: 'lg' }} radius="lg" style={surface}><Stack>
      ...
      <Grid align="flex-end"><Grid.Col span={{ base: 12, sm: 6, lg: 5 }}><Select classNames={{ dropdown: styles.dropdown }} label="EWP" placeholder="Select EWP" data={ewps.map(row => ({ value: row.id, label: `${row.code} · ${row.name}` }))} value={ewpId} searchable clearable disabled={busy} onChange={value => { setEwpId(value); setRegisterId(null); setRegister(null); setOutputs([]); setRegisters([]); setError(''); }} /></Grid.Col>
        <Grid.Col span={{ base: 12, sm: 6, lg: 5 }}><Select classNames={{ dropdown: styles.dropdown }} label="Quantity register" placeholder="Select a take-off" data={registers.map(row => ({ value: row.id, label: `${row.name} · ${row.status.replaceAll('_', ' ')}` }))} value={registerId} searchable disabled={!ewpId || busy} onChange={setRegisterId} /></Grid.Col>
        <Grid.Col span={{ base: 12, lg: 2 }}><Button color="green" fullWidth disabled={!outputs.length || busy} leftSection={<IconPlus size={15} />} onClick={() => { setRegisterName(''); setModal('register'); }}>Create QTO</Button></Grid.Col></Grid>
      ...
      {!!outputs.length && <><Divider /><Text fw={700} size="sm">Released engineering outputs</Text><Box className={styles.tableWrap}><Table className={styles.recordTable}><Table.Thead><Table.Tr><Table.Th>Document / deliverable</Table.Th><Table.Th>Revision</Table.Th><Table.Th>SEB basis</Table.Th><Table.Th>Source details</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{outputs.map(row => <Table.Tr key={row.revision_id}><Table.Td data-label="Document / deliverable"><Box><Text size="sm" fw={650}>{row.document_code} · {row.document_title}</Text><Text size="xs" c="dimmed">{row.deliverable.code} · {row.deliverable.name}</Text></Box></Table.Td><Table.Td data-label="Revision"><Box>{row.revision_no} <Status value="RELEASED" /></Box></Table.Td><Table.Td data-label="SEB basis">{row.seb_basis.seb?.code || row.seb_basis.seb_id} / {row.seb_basis.released_revision?.revision_no || row.seb_basis.revision_id}</Table.Td><Table.Td data-label="Source details"><Button variant="subtle" size="xs" onClick={() => setSource(row)}>View source</Button></Table.Td></Table.Tr>)}</Table.Tbody></Table></Box></>}
    </Stack></Paper>

    {register && <>
      <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Stack>
        ...
        {status === 'DRAFT' && <Group className={styles.actions} justify="flex-end"><Button color="green" leftSection={<IconPlus size={15} />} disabled={saving || !outputs.length} onClick={() => edit(null)}>Add quantity item</Button></Group>}
      ...
      {!!register.boqs.length && <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Stack><Group justify="space-between"><Text fw={750}>BOQ / EBOM register</Text>{['APPROVED', 'PROCUREMENT_READY', 'HANDOFF_REFERENCED'].includes(status) && <Button variant="light" color="green" onClick={download}>Download approved package</Button>}</Group>
      ...
      {['PROCUREMENT_READY', 'HANDOFF_REFERENCED'].includes(status) && <Paper withBorder radius="lg" p={{ base: 'sm', sm: 'lg' }} style={surface}><Stack>
  ...
  <Modal fullScreen={isMobile} classNames={{ content: styles.modal }} closeButtonProps={{ "aria-label": "Close dialog" }} opened={modal === 'item'} onClose={() => !saving && setModal('')} title={editingItem ? 'Edit quantity item' : 'Add quantity item'} size="lg" centered><Stack>
    ...
    <SimpleGrid cols={{ base: 1, sm: 2 }}><NumberInput label="Quantity" required min={0} value={itemForm.quantity} onChange={value => updateItem('quantity', value)} /><TextInput label="Unit" required value={itemForm.unit} onChange={event => updateItem('unit', event.currentTarget.value)} /></SimpleGrid>
    <Select classNames={{ dropdown: styles.dropdown }} label="Released document revision" required searchable placeholder="Select source" value={itemForm.source_revision_id} data={outputs.map(row => ({ value: row.revision_id, label: `${row.document_code} / ${row.revision_no} · ${row.document_title}` }))} onChange={value => updateItem('source_revision_id', value)} />
  ...
  <Modal fullScreen={isMobile} classNames={{ content: styles.modal }} closeButtonProps={{ "aria-label": "Close dialog" }} opened={Boolean(labels[modal])} onClose={() => !saving && setModal('')} title={labels[modal]} centered size="lg"><Stack>
    ...
    {modal === 'generate' && <Select classNames={{ dropdown: styles.dropdown }} label="Quantity set type" data={['BOQ', 'EBOM']} value={form.type} onChange={value => updateForm('type', value)} />}
    {['review', 'approve'].includes(modal) && <Select classNames={{ dropdown: styles.dropdown }} label="Decision" data={(modal === 'review' ? ['ACCEPT', 'CHANGE_REQUIRED'] : ['APPROVE', 'CHANGE_REQUIRED', 'REJECT']).map(value => ({ value, label: value.replaceAll('_', ' ') }))} value={form.decision} onChange={value => updateForm('decision', value)} />}
    ...
    {modal === 'tracking' && <Select classNames={{ dropdown: styles.dropdown }} label="Reported status" data={(TRACKING[handoff?.status] || []).map(value => ({ value, label: value.replaceAll('_', ' ') }))} value={form.status} onChange={value => updateForm('status', value)} />}
    ...
    <Group className={styles.actions} justify="flex-end"><Button variant="default" disabled={saving} onClick={() => setModal('')}>Cancel</Button><Button color="green" loading={saving} disabled={actionInvalid} onClick={submitAction}>Save {modal === 'tracking' ? 'status' : modal === 'handoff' ? 'reference' : 'decision'}</Button></Group>
  </Stack></Modal>

  <Modal fullScreen={isMobile} classNames={{ content: styles.modal }} closeButtonProps={{ "aria-label": "Close dialog" }} opened={!!viewBoq} onClose={() => setViewBoq(null)} title={`${viewBoq?.type || ''} / ${viewBoq?.revision_no || ''}`} size="xl" centered><Stack>{viewBoq && <><Status value={viewBoq.status} /><Field label="Quantity content hash" value={viewBoq.content_hash} /><QuantityTable rows={viewBoq.items} onSource={setSource} /></>}</Stack></Modal>
  <Modal fullScreen={isMobile} classNames={{ content: styles.modal }} closeButtonProps={{ "aria-label": "Close dialog" }} opened={!!source} onClose={() => setSource(null)} title="Released engineering source" size="lg" centered><Stack>{source && <>
